import { paths, renameArg } from "../../types/types";

const ipcRenderer = require("electron").ipcRenderer;
let songPath: string;

ipcRenderer.on("song-name", (_, arg: paths) => {
    songPath = arg.songPath;
    document.getElementById("original-song-name").innerHTML = arg.songName;
});

document.getElementById("song-rename-cancel-button").addEventListener("click", (_) => {
    window.close();
});

document.getElementById("song-rename-accept-button").addEventListener("click", (event) => {
    const requestedName = (document.getElementById("song-rename-newName") as HTMLTextAreaElement).value;
